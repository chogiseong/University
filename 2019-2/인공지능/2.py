...
ty=np.reshape(ty,[576,1]).astype(np.float32)

#tensorflow
X=tf.placeholder(tf.float32,shape=[576,9])
Y=tf.placeholder(tf.float32,shape=[576,1])

W = tf.Variable(tf.random_uniform([9, 100],  -0.5,0.5))
B = tf.Variable(tf.random_uniform([100], -0.5,0.5))
hiddenLayer1 = tf.sigmoid( tf.add(tf.matmul(X, W), B) )

W2 = tf.Variable(tf.random_uniform([100, 50],  -0.5,0.5))
B2 = tf.Variable(tf.random_uniform([50], -0.5,0.5))
##hiddenLayer  = tf.nn.relu(tf.matmul(X,W) + B)
hiddenLayer2 = tf.sigmoid( tf.add(tf.matmul(hiddenLayer1, W2), B2) )

W3  = tf.Variable(tf.random_uniform([50,2],-0.5,0.5))
...
hiddenLayerResult=sigmoid(np.add(np.dot(testx,w2),b2)).astype(np.float32)
outLayerResult=sigmoid(np.add(np.dot(hiddenLayerResult,w3),b3)).astype(np.float32)
test_prediction=np.zeros(383).astype(np.float32)
